...
<section data-page="dashboard" class="space-y-6 text-[13px]">
  <style>
  /* Tema claro: cards blancas, bordes suaves y texto oscuro */
  [data-page="dashboard"] .glass{ background:#ffffff; box-shadow:0 1px 2px rgba(2,6,23,0.04); }
  [data-page="dashboard"] .card{ border:1px solid rgba(2,6,23,0.06); border-radius:.6rem; background:transparent; }
  [data-page="dashboard"] .k{ font-size:.74rem; color:#6b7280; }
    [data-page="dashboard"] .v{ font-size:1.06rem; font-weight:700; letter-spacing:.2px; color:#0f172a }
    [data-page="dashboard"] .g-indigo{ background:linear-gradient(135deg,rgba(99,102,241,.08),rgba(99,102,241,.02)); }
    [data-page="dashboard"] .g-emerald{ background:linear-gradient(135deg,rgba(16,185,129,.06),rgba(16,185,129,.02)); }
    [data-page="dashboard"] .g-pink{ background:linear-gradient(135deg,rgba(236,72,153,.06),rgba(236,72,153,.02)); }
    [data-page="dashboard"] .g-cyan{ background:linear-gradient(135deg,rgba(6,182,212,.06),rgba(6,182,212,.02)); }
    [data-page="dashboard"] .g-amber{ background:linear-gradient(135deg,rgba(245,158,11,.06),rgba(245,158,11,.02)); }
    [data-page="dashboard"] .g-slate{ background:linear-gradient(135deg,rgba(100,116,139,.06),rgba(100,116,139,.02)); }
    ...
    [data-page="dashboard"] .ctrl{ background:#fff; border:1px solid rgba(2,6,23,0.06); color:#0f172a }
    [data-page="dashboard"] .btn{ display:inline-flex; align-items:center; gap:.4rem; padding:0 .7rem; border-radius:.45rem; border:1px solid rgba(2,6,23,0.06); background:#fff; color:#0f172a }
    [data-page="dashboard"] .btn:hover{ box-shadow:0 6px 16px rgba(15,23,42,0.04) }
    [data-page="dashboard"] .btn-primary{ background:linear-gradient(180deg,#2563eb,#1d4ed8); color:#fff; border-color:transparent }
    [data-page="dashboard"] .btn-primary:hover{ filter:brightness(.98) }
    [data-page="dashboard"] .btn-info{ background:linear-gradient(180deg,#06b6d4,#0891b2); color:#fff; border-color:transparent }
    [data-page="dashboard"] .btn-info:hover{ filter:brightness(.98) }
    [data-page="dashboard"] .btn-icon{ height:28px; line-height:26px; padding:0 .45rem; font-size:12px; background:transparent; border:1px solid rgba(2,6,23,0.04) }

    [data-page="dashboard"] .table-wrap{ border:1px solid rgba(2,6,23,0.04); border-radius:.5rem; overflow:hidden; background:#fff }
    [data-page="dashboard"] .table td, .table th{ padding:.5rem .65rem; border-bottom:1px solid rgba(2,6,23,0.04) }
    [data-page="dashboard"] th{ font-weight:600; color:#0f172a; white-space:nowrap; }

    [data-page="dashboard"] .badge{ font-size:.68rem; padding:.16rem .45rem; border-radius:.45rem }
    [data-page="dashboard"] .status-active{ background:rgba(16,185,129,0.12); color:#065f46 }
    [data-page="dashboard"] .status-expired{ background:rgba(220,38,38,0.08); color:#9b1c1c }

    [data-page="dashboard"] select option{ color:#0f172a; background-color:#fff; }
    [data-page="dashboard"] .icon{ font-size:1.05rem; color:#0f172a }
  </style>
  ...
  <div class="grid xl:grid-cols-2 gap-4">
    ...
    <div class="glass card p-4 space-y-2">
      <div class="flex items-center justify-between">
        <h2 class="font-medium text-[13px] leading-none">Presupuestos no ejecutados</h2>
        <div class="text-[12px] text-slate-500" id="unexecuted-total">—</div>
      </div>
      <div id="unexecuted-list" class="text-[12.5px] space-y-2 max-h-44 overflow-auto"></div>
      <div id="unexecuted-empty" class="text-slate-400 text-xs py-2 hidden">Sin presupuestos no ejecutados.</div>
    </div>
  </div>
</section>
    `;
        ...
        return `
          <tr class="hover:bg-white/5">
            ...
            <td class="text-right whitespace-nowrap">
              ...
              <button data-act="del" data-key="${b.key}" data-num="${b.numero}" class="btn btn-icon" title="Eliminar"><i class="fas fa-trash" aria-hidden="true"></i></button>
            </td>
          </tr>
        `;
      }).join("");
